...
class SearchEngine:
    ...
    def build_tf_matrix(self):
        ...
        for i in range(1, self.corpus.ndoc + 1):
            ...
            motsFreq = {
                mot: mots.count(mot) for mot in set(mots)
            }  # Compter occurrences de chaque mot dans le document
        ...
        self.mat_TF = csr_matrix(
            (data, (rows, cols)), shape=(self.corpus.ndoc, len(self.vocab))
        )
    ...
    def compute_tfidf(self):
        ...
        N = self.corpus.ndoc
        frequence_document = np.array(
            (self.mat_TF > 0).sum(axis=0)
        ).flatten()  # Nombre de documents contenant chaque mot
