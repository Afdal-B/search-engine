...
#===================TD6======================
    ...
    def search(self, keyword, context_size):
        ...
        right_context = self._searchString[index+len(keyword):min(index+len(keyword)+context_size,len(self._searchString))]
 ...
 # La fonction concorde
    def concorde(self,keyword,context_size):
        ...
        for i in indices:
            tab["contexte gauche"].append(self._searchString[max(0, i-context_size):i])
            tab["motif trouve"].append(self._searchString[i:i+len(keyword)]) 
            tab["contexte droit"].append(self._searchString[i+len(keyword):min(i+len(keyword)+context_size,len(self._searchString))])
    ...
    def stats(self, n=10):
        ...
        mots_plus_frequents = freq_df.sort_values(by='Term Frequency', ascending=False).head(n)
    ...
    def genererFreq(self):
        ...
        df = DataFrame.from_dict(self.freq, orient='index', columns=['Term Frequency', 'Document Frequency'])
        return df
